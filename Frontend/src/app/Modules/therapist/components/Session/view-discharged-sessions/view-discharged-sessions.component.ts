...
@Component({
  selector: 'app-view-discharged-sessions',
  templateUrl: './view-discharged-sessions.component.html',
  styleUrls: ['./view-discharged-sessions.component.scss']
})
export class ViewDischargedSessionsComponent implements OnInit, OnDestroy {
  ...
  dischargedSessions : SessionMaster[] = [];
  id : any;
  sessionMS: Subscription;
  getSessionMaster(){
    this.sessionMS = this.therapistService.getSessionMaster()
      .pipe(map((x: SessionMaster[]) =>
          x.filter((y) => y.sessionStatus === 'Session' &&
                          y.session_id.status == true
          )
        ))
        .subscribe((s) => {
        const session = s;
